...
<p align="center">
  <strong>自动将目标转化为任务依赖图（DAG）。</strong><br/>
  TypeScript 原生多智能体编排。仅需三个运行时依赖。
</p>
...
`open-multi-agent` 是一个面向 TypeScript 后端的智能体（Agent）编排框架。给它一个目标，协调员智能体会将其分解为任务依赖图（DAG），并行执行独立子任务并汇总结果。仅需三个运行时依赖即可无缝接入任何 Node.js 后端。

> **你的工程师只需描述目标，无需手动绘制流程图。**

基于图优先（Graph-first）的框架要求你预先枚举所有节点和边。`open-multi-agent` 采用目标优先（Goal-first）设计：你只需描述期望的结果，协调器会在运行时动态构建任务依赖图，使编排逻辑随目标自适应变化，而非硬编码固定流程。

## Contents

[快速开始](#quick-start) · [三种运行方式](#three-ways-to-run) · [核心特性](#features) · [编排控制](#orchestration-controls) · [生态系统](#ecosystem) · [示例](#examples) · [与同类项目的区别](#how-is-this-different-from-x) · [架构设计](#architecture) · [支持的提供商](#supported-providers) · [生产环境检查清单](#production-checklist) · [文档](#documentation) · [贡献指南](#contributing)
...
*从 `@jackchen_me/open-multi-agent` 迁移？该包已废弃，请改为安装 `@open-multi-agent/core`。*
...
三个智能体协作完成 REST API 的开发，同时 `onProgress` 会实时流式输出协调器的任务依赖图：
...
通过 Ollama 使用的本地模型无需配置 API Key，详见 [`providers/ollama`](examples/providers/ollama.ts)。对于托管服务提供商（需设置 `OPENAI_API_KEY`、`GEMINI_API_KEY` 等），请参阅 [支持的提供商](#supported-providers)。
