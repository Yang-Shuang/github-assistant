...
> **将任意网站转换为命令行界面，并在已登录的 Chrome 中运行浏览器自动化操作（Browser Use）。**
> 将网站、浏览器会话、Electron 应用和本地工具转化为供人类和 AI Agent（智能体）使用的确定性接口。
> 或针对任意页面执行浏览器自动化操作——导航、填写表单、点击、提取数据、实现自动化。
...
OpenCLI 为你提供一个统一的入口，支持三种不同类型的自动化操作：

- **使用内置适配器** 访问哔哩哔哩、知乎、小红书、Reddit、Hacker News、Twitter/X 等网站，以及[更多站点](#built-in-commands)。
- **让 AI Agent 操作任意网站** —— 在你的 AI Agent（如 Claude Code、Cursor 等）中安装 `opencli-browser` 技能，它即可通过 `opencli browser` 基础命令，利用你已登录的浏览器进行导航、点击、输入/填写、提取数据和检查页面。
- **编写全新适配器** —— 结合 `opencli browser` 与 `opencli-adapter-author` 技能，从首次侦察、字段解析、代码编写到 `opencli browser verify`（验证）实现端到端开发。

它同时也是一个 **CLI 中枢**，可统一管理 `gh`、`docker`、`longbridge`、`tg`、`discord`、`wx`、`ntn`（Notion）等本地工具，以及你自行注册的二进制文件；此外还提供针对 Cursor、Trae CN、Codex、Antigravity、ChatGPT 和 Trae SOLO 等 Electron 应用的**桌面应用适配器**。
