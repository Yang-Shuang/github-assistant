...
[![README en 한국어][lang-kr-badge]][lang-kr-readme]
...
- [智能体示例][agent-examples-section]
- [快速开始：运行智能体示例][quick-start]
  - [本地环境 (Localhost)][localhost-section]
  - [GitHub Codespaces][codespaces-section]
- [智能体示例自托管部署][agent-examples-self-hosting]
...
TEN 是一个用于实时多模态对话式人工智能的开源框架。

[TEN 生态系统 (TEN Ecosystem)][ten-ecosystem-anchor] 包含 [TEN Framework][ten-framework]、[智能体示例 (Agent Examples)][agent-examples-repo]、[VAD（语音活动检测）][ten-vad]、[话轮检测 (Turn Detection)][ten-turn-detection] 和 [Portal][ten-portal]。

<br>

| 社区渠道 | 用途 |
| ---------------- | ------- |
| [![Follow on X][follow-on-x-badge]][follow-on-x] | 在 X (原 Twitter) 上关注 TEN Framework，获取最新动态与公告 |
| [![Discord TEN Community][discord-badge]][discord-invite] | 加入我们的 Discord 社区，与开发者交流互动 |
| [![Follow on LinkedIn][linkedin-badge]][linkedin] | 在 LinkedIn 上关注 TEN Framework，获取最新动态与公告 |
| [![Hugging Face Space][hugging-face-badge]][hugging-face] | 加入我们的 Hugging Face 社区，探索相关空间与模型 |
| [![WeChat][wechat-badge]][wechat-discussion] | 加入我们的微信群，参与中文社区讨论 |
...
<strong>多功能语音助手</strong> — 这款低延迟、高质量的实时助手支持 RTC 和 [WebSocket][websocket-example] 连接，你可以通过 [Memory（记忆）][memory-example]、[VAD（语音活动检测）][voice-assistant-vad-example]、[话轮检测 (Turn Detection)][voice-assistant-turn-detection-example] 以及其他扩展功能对其进行增强。
...
<strong>Doodler（涂鸦画板）</strong> — 一个涂鸦画板，可将语音或输入的提示词转化为简单的手绘草图，内置蜡笔调色盘并支持实时绘制。
...
<strong>Speaker Diarization（说话人识别）</strong> — 实时检测并标注说话人的声纹识别功能，“Who Likes What”游戏展示了交互式应用场景。
...
<strong>Lip Sync Avatars（口型同步虚拟形象）</strong> — 兼容多个虚拟形象供应商，主角为动漫角色 Kei（采用 MotionSync 技术实现口型同步），同时支持来自 Trulience、HeyGen 和 Tavus 的写实风格虚拟形象。

不同 Live2D 角色的代码请参阅 [示例代码][voice-assistant-live2d-example]。
...
<strong>SIP Call（SIP 通话）</strong> — 基于 TEN 驱动的 SIP 扩展，支持电话呼叫功能。
...
<strong>Transcription（语音转写）</strong> — 一款将音频转换为文本的转写工具。
...
<strong>ESP32-S3 Korvo V3</strong> — 在乐鑫 ESP32-S3 Korvo V3 开发板上运行 TEN 智能体示例，实现大语言模型驱动的通信与硬件集成。
...
获取新版本与更新的即时通知。你的支持有助于我们不断成长并完善 TEN！
...
[localhost-49483-image]: https://github.com/user-attachments/assets/19a7c0a-d8e6-48f9-866f-6a70c58f0118
...
[ten-turn-detection-banner]: https://github.com/user-attachments/assets/c72d84cc-3667-496c-8bd6-3d194a91c452
...
[ten-portal-banner]: https://github.com/user-attachments/assets/f56c75b9-72c8-4156-902d-ae98ce2b3b5e
