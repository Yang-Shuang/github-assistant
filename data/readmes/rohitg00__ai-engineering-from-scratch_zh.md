...
> **已有 84% 的学生使用过 AI 工具，但只有 18% 的人认为自己能专业地使用它们。** 本课程旨在填补这一差距。
>
> 503 课时。20 个阶段。约 320 小时。涵盖 Python、TypeScript、Rust、Julia。每节课都会交付一个可复用的产物：提示词（prompt）、技能（skill）、智能体（agent）或 MCP 服务器。免费开源，MIT 协议。
>
> 你不仅是在学习 AI，更是在亲手构建它。端到端，从零开始。

## 工作原理 (How this works)

大多数 AI 资料的教学内容都是零散的。这篇讲论文，那篇讲微调（fine-tuning），别处又有个炫酷的智能体演示。这些碎片很少能拼凑在一起。你搭建了一个聊天机器人，却解释不清它的损失曲线（loss curve）；你把一个函数接入智能体，却说不出调用它的模型内部注意力机制（attention）是如何工作的。

本课程就是那条脊柱（核心主线）。20 个阶段，503 课时，四种语言：Python、TypeScript、Rust、Julia。一端是线性代数，另一端是自洽的智能体集群。每个算法都先从原始数学公式开始构建。反向传播、分词器、注意力机制、智能体循环。等到 PyTorch 出现时，你已经清楚它底层在做什么了。

每节课都遵循相同的流程：阅读问题、推导数学公式、编写代码、运行测试、保留产物。没有五分钟的视频，没有复制粘贴的部署步骤，也没有手把手的教学。免费开源，专为在你的笔记本电脑上运行而设计。
...
二十个阶段层层递进。数学是地基，智能体与生产部署是屋顶。如果你已经掌握底层知识，可以跳级学习，但请不要跳过基础却在上层遇到报错时感到困惑。

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#fafaf5','primaryTextColor':'#1a1a1a','primaryBorderColor':'#3553ff','lineColor':'#3553ff','fontFamily':'JetBrains Mono','fontSize':'12px'}}}%%
flowchart TB
  P0["阶段 0 — 设置与工具链"] --> P1["阶段 1 — 数学基础"]
  P1 --> P2["阶段 2 — 机器学习基础"]
  P2 --> P3["阶段 3 — 深度学习核心"]
  P3 --> P4["阶段 4 — 计算机视觉"]
  P3 --> P5["阶段 5 — 自然语言处理"]
...
## 课时结构 (The shape of a lesson)

每节课都拥有独立的文件夹，整个课程的结构保持一致：
...
每节课遵循六个步骤。*构建它 / 使用它（Build It / Use It）*是核心主线——你先从零实现算法，再通过生产级库运行相同的功能。因为你亲手写过更基础的版本，所以你能理解框架底层在做什么。

```mermaid
%%{init: {'theme':'base','themeVariables':{'primaryColor':'#fafaf5','primaryTextColor':'#1a1a1a','primaryBorderColor':'#3553ff','lineColor':'#3553ff','fontFamily':'JetBrains Mono','fontSize':'13px'}}}%%
flowchart LR
  M["MOTTO<br/><sub>核心一句话理念</sub>"] --> Pr["PROBLEM<br/><sub>具体痛点</sub>"]
  Pr --> C["CONCEPT<br/><sub>图解与直觉</sub>"]
  C --> B["BUILD IT<br/><sub>原始数学，无框架依赖</sub>"]
  B --> U["USE IT<br/><sub>使用 PyTorch / sklearn 实现相同功能</sub>"]
...
**选项 A：在线阅读**。打开 [aiengineeringfromscratch.com](https://aiengineeringfromscratch.com) 上的任意已完成课程，或展开[目录](#contents)下的某个阶段。无需设置，无需克隆仓库。
...
**选项 C：定位你的水平（推荐）** 智能跳过。在 Claude、Cursor、Codex、OpenClaw、Hermes 或任何已安装课程技能的 agent 中执行：

```bash
/find-your-level
```

十个问题。将你的知识映射到起始阶段，并生成带有小时估算的个性化路径。每个阶段结束后：

```bash
/check-understanding 3        # 针对阶段 3 进行自测
...
- 你会写代码（任何语言均可；懂 Python 更好）。
- 你想了解 AI **真正的工作原理**，而不仅仅是调用 API。

### 内置 agent 技能（Claude, Cursor, Codex, OpenClaw, Hermes）

| 技能 | 作用 |
|---|---|
| [`/find-your-level`](.claude/skills/find-your-level/SKILL.md) | 十题水平测试。将你的知识映射到起始阶段，并生成带有小时估算的个性化路径。 |
| [`/check-understanding <phase>`](.claude/skills/check-understanding/SKILL.md) | 分阶段测验，共八道题，附带反馈和需要复习的具体课程建议。 |

```
░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒
```

## 每节课都会交付产物 (Every lesson ships something)

其他课程以“恭喜你，学会了 X”结束。这里的每节课都以一个**可复用的工具**结尾，你可以将其安装或粘贴到日常工作中。
...
<tr>
<td valign="top">粘贴到任意 AI 助手，用于特定领域的专家级辅助。</td>
<td valign="top">放入 Claude、Cursor、Codex、OpenClaw、Hermes 或任何读取 <code>SKILL.md</code> 的 agent 中即可使用。</td>
<td valign="top">作为自主工作者部署——你在阶段 14 亲手编写了循环逻辑。</td>
<td valign="top">接入任意兼容 MCP 的客户端。在阶段 13 端到端构建完成。</td>
</tr>
</table>

> 使用 `python3 scripts/install_skills.py` 一键安装所有技能。这是真正的工具，不是作业。课程结束时，你将拥有一个包含 503 个产物的作品集，并且因为是你亲手构建的，所以你真正理解它们。
...
每个阶段 14 的工作台课时（31-42）都会附带一份 `mission.md`，在打开完整课程文档前向 agent 下达任务简报。
...
**Deep-build tracks（深度构建系列）** — 多课时专题，从零构建完整的子系统。
...
每节课都会产出可复用的产物。结束时你将拥有：

```
outputs/
├── prompts/      适用于各类 AI 任务的提示词模板
└── skills/       AI 编程 agent 使用的 SKILL.md 文件
```

使用 `npx skills add` 安装它们。将它们接入 Claude、Cursor、Codex、OpenClaw、Hermes，或任何读取 SKILL.md / AGENTS.md 目录的 agent。这是真正的工具，不是作业。

### 将课程技能一键安装到你的 agent 中

仓库在 `phases/**/outputs/` 下提供了 388 个技能和 99 个提示词。

**推荐：通过 [skills.sh](https://skills.sh) 安装。** 无需克隆，无需 Python，会自动检测你的 agent 技能目录：
...
`skills` 会写入你的 agent 识别的目录：`.claude/skills/`、`.cursor/skills/`、`.codex/skills/`、OpenClaw 的技能文件夹、Hermes 的 bundle 路径，或任何支持 SKILL.md 的工具。一条命令，适配所有 agent。

**高级用法：通过 `scripts/install_skills.py` 离线安装 / 自定义布局。** 需要克隆仓库。当你需要标签过滤、试运行或非默认布局时非常有用：
...
python3 scripts/install_skills.py <target> --dry-run                       # 预览不写入
python3 scripts/install_skills.py <target> --force                         # 覆盖现有文件
```

`<target>` 是你的 agent 技能目录（示例：`~/.claude/skills/`、`~/.cursor/skills/`、`~/.config/openclaw/skills/`、`.skills/`，或你的 agent 读取的任何路径）。

默认情况下，脚本会拒绝覆盖现有目标文件，并在列出所有冲突路径后以退出码 1 退出。使用 `--dry-run` 预览冲突，或使用 `--force` 强制覆盖。每次非试运行都会在目标目录下写入一个 `manifest.json`，按类型和阶段分组记录完整清单。选择你的 agent 读取的布局：
...
阶段 14 的综合项目提供了一个可复用的 Agent Workbench 包（包含 AGENTS.md、schemas、初始化/验证/交接脚本）。使用以下命令将其搭建到任意仓库中：
...
python3 scripts/scaffold_workbench.py path/to/your-repo --force    # 覆盖写入
```

你将获得已配置好的七个工作台表面、一个 starter `task_board.json`，以及位于 `schema_version: 1` 的崭新 `agent_state.json`。之后：编辑任务、修改 `AGENTS.md`、运行 `scripts/init_agent.py`、将契约交给你的 agent。该包的源码位于
`phases/14-agent-engineering/42-agent-workbench-capstone/outputs/agent-workbench-pack/`。

### 以 JSON 格式浏览整个课程

`scripts/build_catalog.py` 会遍历磁盘上的每个阶段、每节课和每个产物，并在仓库根目录写入 `catalog.json`。一个文件，涵盖全部课程数据。
...
该目录基于文件系统生成（而非 README），因此统计数字始终与实际磁盘内容一致。可用于站点构建、下游工具开发，或验证 README 中的计数是否已发生偏移。Schema 文档位于脚本顶部。

GitHub Action (`.github/workflows/curriculum.yml`) 会在每次 PR 时重建 `catalog.json`，并在提交文件过时阻止构建。编辑任何课程后，运行 `python3 scripts/build_catalog.py` 并提交结果，否则 CI 将拒绝该 PR。同一工作流会以 warn-only 模式运行 `audit_lessons.py`（以便现有偏移不会阻碍贡献者）。

### 冒烟测试每节课的 Python 代码

`scripts/lesson_run.py` 会对每个课时 `code/` 目录下的所有 `.py` 文件进行字节编译。默认模式仅为语法检查——无需执行、无需 API key、无需重型 ML 依赖。可捕获贡献者最常引入的回归问题（错误缩进、损坏的 f-string、意外修改）。

```bash
python3 scripts/lesson_run.py                  # 语法检查整个课程
python3 scripts/lesson_run.py --phase 14       # 仅单个阶段
python3 scripts/lesson_run.py --json           # stdout JSON 报告
python3 scripts/lesson_run.py --strict         |exit 1| 如果任何课时失败则退出码为 1
python3 scripts/lesson_run.py --execute        # 实际运行，每节课超时 10s
```

`--execute` 会运行每节课的 `code/main.py`（或第一个 `.py` 文件），并设置 10 秒超时。条目文件以 `# requires: pkg1, pkg2` 注释开头且列出了非标准库依赖的课程，会被跳过并注明原因 `needs <deps>`。该脚本为可选功能，未接入 CI。

仅支持标准库，Python 3.10+。设置 `LINK_CHECK_SKIP=domain1,domain2` 可覆盖默认跳过列表（`twitter.com`、`x.com`、`linkedin.com`、`instagram.com`、`medium.com`——这些域名会 aggressively 阻止自动化的 HEAD/GET 请求）。

## 从哪里开始 (Where to start)

| 背景知识 | 起始阶段 | 预估时间 |
|---|---|---|
| 编程与 AI 新手 | Phase 0 — Setup & Tooling | ~306 hours |
| 熟悉 Python，机器学习新手 | Phase 1 — Math Foundations | ~270 hours |
| 熟悉 ML，深度学习新手 | Phase 3 — Deep Learning Core | ~200 hours |
| 熟悉深度学习，想学 LLM 和 agent | Phase 10 — LLMs from Scratch | ~100 hours |
| 高级工程师，仅需 Agent Engineering | Phase 14 — Agent Engineering | ~60 hours |

```
░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒
```

## 为什么现在很重要 (Why this matters now)

<table>
<tr>
<th align="left" width="50%"><sub>FIG_003 · A</sub><br/><b>行业风向标</b></th>
...
| Fork 用于团队或学校 | [FORKING.md](FORKING.md) |
| 课时模板 | [LESSON_TEMPLATE.md](LESSON_TEMPLATE.md) |
...
当任何规则校验失败时，退出码为非零。规则（L001–L010）会验证目录结构、`docs/en.md` 的存在及 H1 标题、`code/` 非空、`quiz.json` schema（拒绝导致 issue #102 的旧版 `q/choices/answer` 键），以及课程文档内的相对链接。

```
░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒
```

## 赞助本项目 (Sponsor the work)

免费，MIT 协议，503 课时。本课程仅靠赞助维护。仅限现金支付。

**影响力（2026-05-14 已验证）：** 月访问量 55,593 · 页面浏览量 90,709 · Stars 7.5K · Twitter/X 是首要流量来源渠道。

**当前赞助方：** [CodeRabbit](https://coderabbit.link/rohit-ghumare) · [iii](https://iii.dev?utm_source=ai-engineering-from-scratch&utm_medium=readme&utm_campaign=sponsor)

| 等级 | $/月 | 权益 |
|------|------|---|
| Backer | $25 | 姓名列入 BACKERS.md |
| Bronze | $250 | README 赞助区块中的纯文本行 + 发布日推文 |
| Silver | $750 | README 中小 logo + API 课程中标记为支持提供商之一 |
| Gold | $2,000 | README 中中等 logo + 赞助页 + 每季度 X / LinkedIn 联合推广 |
| Platinum | $5,000 | 首屏 Hero logo + 专属集成课时，限 1 个合作伙伴 |

完整价目表、硬性规则、定价锚点及影响力数据：[SPONSORS.md](SPONSORS.md)。通过 [GitHub Sponsors](https://github.com/sponsors/rohitg00) 注册。

```
░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒░░░▒▒▒
```

## Stars 历史 (Star history)
...
如果本手册对你有帮助，请给仓库点星。这将帮助项目持续运转。

## License (许可证)

MIT。随意使用——Fork、教学、售卖或部署均可。建议注明出处，但不强制。

由 [Rohit Ghumare](https://github.com/rohitg00) 及社区共同维护。
