...
<p align="center">面向 AI Agent 和 RAG（检索增强生成）系统的开源上下文检索层。</p>
...
### 什么是 Airweave？
Airweave 连接你的应用、工具和数据库，持续同步其数据，并通过统一且对大语言模型（LLM）友好的搜索接口暴露这些数据。AI Agent 通过查询 Airweave，在单次请求中即可从多个来源检索到相关、有据可依且实时的上下文信息。

### 架构定位
Airweave 作为共享检索基础设施，部署在你的数据源与 AI 系统之间。它负责处理身份验证、数据摄入（ingestion）、同步、索引和检索，让你无需为每个 Agent 或集成场景重复构建脆弱的数据管道。

### 工作原理
1. **连接（Connect）**你的应用、数据库和文档（支持 50+ 种集成）
2. **Airweave** 通过统一的检索层对你的数据进行同步、索引并暴露接口
3. **Agent 查询（Agents query）** Airweave，可通过我们的 SDK、REST API、MCP 协议或与主流 Agent 框架的原生集成进行调用
4. **Agent 按需检索（Agents retrieve）** 相关且可靠的上下文信息

## 快速开始

### 云托管版：[app.airweave.ai](https://app.airweave.ai)
...
该 CLI 工具会在终端输出丰富的交互式结果，并通过管道（pipe）传递时输出纯净的 JSON —— 使其既能满足开发者的使用习惯，也能被 AI Agent 直接解析调用。
