...
一个模型上下文协议（Model Context Protocol，简称 MCP）服务器，将 [Firecrawl](https://github.com/firecrawl/firecrawl) 集成到支持 MCP 的 AI Agent 中——用于搜索、抓取并与实时网页交互，为 Agent 提供干净、可直接使用的上下文。

> 特别感谢 [@vrknetha](https://github.com/vrknetha) 和 [@knacklabs](https://www.knacklabs.ai) 完成了初始实现！
...
- 与页面交互——点击、导航和操作
- 通过自主 Agent 进行深度研究
- 自动重试与速率限制（Rate Limiting）
- 支持云端与自托管部署
- 支持 SSE（Server-Sent Events）

> 你可以在 [MCP.so 的 MCP Server 游乐场](https://mcp.so/playground?server=firecrawl-mcp-server) 或 [Klavis AI](https://www.klavis.ai/mcp-servers) 上体验我们的 MCP Server。
...
### Running on Cursor（在 Cursor 中运行）

Configuring Cursor 🖥️
注意：需要 Cursor 版本 0.45.6 及以上。如需最新的配置说明，请参阅官方文档中关于配置 MCP Server 的部分：
[Cursor MCP Server Configuration Guide](https://docs.cursor.com/context/model-context-protocol#configuring-mcp-servers)

To configure Firecrawl MCP in Cursor **v0.48.6**（在 Cursor **v0.48.6** 中配置）
...
To configure Firecrawl MCP in Cursor **v0.45.6**（在 Cursor **v0.45.6** 中配置）
...
### Running on Windsurf（在 Windsurf 中运行）
...
### Running with Streamable HTTP Local Mode（通过流式 HTTP 本地模式运行）
...
### Installing via Smithery (Legacy)（通过 Smithery 安装，已弃用）
...
### Running on VS Code（在 VS Code 中运行）
...
#### Required for Cloud API（云端 API 必需项）
...
##### Credit Usage Monitoring（额度使用监控）
...
### Usage with Claude Desktop（在 Claude Desktop 中使用）
...
2. **Credit Usage Monitoring（额度使用监控）**
...
### Rate Limiting and Batch Processing（速率限制与批处理）
