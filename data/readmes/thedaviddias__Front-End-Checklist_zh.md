...
Front-End Checklist 是一个面向人类和 AI Agent 的开源前端质量审查体系。它将前端最佳实践转化为实用的审查工作流，你可以直接在网页上浏览、通过兼容 MCP 的工具运行，或在本 README 中直接查阅。

- Website: [frontendchecklist.io](https://frontend-front-end-checklist.io)
...
配套项目：[UX Patterns for Devs](https://uxpatterns.dev/) 帮助开发者在使用 Front-End Checklist 验证实现质量之前，先选择正确的 UI 模式。

> [!IMPORTANT]
> 浏览和筛选请使用网站，Agent 工作流请使用 MCP server，若希望将所有检查项集中在一个文件中查阅则使用本 README。

## 你将获得什么

- `385` 条英文规则，涵盖 `11` 个活跃分类
- 托管服务器提供的 `11` 个 MCP 工具
- 包含详细说明、修复指南和验证步骤的规则页面

## 如何使用本检查清单

1. 从下方的分类目录开始，直接跳转到你需要的部分。
2. 根据你的项目、审计或 Pull Request，逐一勾选适用的复选框项。
3. 需要完整指南、示例、验证步骤和 AI Prompt 时，打开链接对应的规则页面。
4. 使用 [frontendchecklist.io](https://frontendchecklist.io) 进行交互式浏览；当你希望 Agent 直接调用相同的规则库时，访问 [mcp.frontendchecklist.io](https://mcp.frontendchecklist.io)。

## 优先级图例

- ![Critical][critical_img] 表示破坏性功能、合规敏感或安全敏感问题，应优先修复。
- ![High][high_img] 表示对用户体验、无障碍性、性能或可发现性有重大影响的问题。
- ![Medium][medium_img] 表示重要的最佳实践，应纳入常规前端质量审查流程。
- ![Low][low_img] 表示有用但具有情境依赖性或紧迫性较低的功能改进。

## 选择你的工作流

### 在线浏览

- 在 [frontendchecklist.io/rules](https://frontendchecklist.io/rules) 探索所有规则
- 使用 [frontendchecklist.io/checklists](https://frontendchecklist.io/checklists) 中的精选清单
- 打开分类页面进行针对性审计和实现指导

### 先选择合适的模式

Front-End Checklist 帮助你审查实现质量。如果你仍在决定要构建哪种界面，请使用 [UX Patterns for Devs](https://uxpatterns.dev/) 来对比常见的 UI 模式、理解权衡取舍，并为表单、导航、数据展示、反馈状态、认证和 AI 界面获取实用指南。

### 贡献检查清单

- 安装依赖：`pnpm install`
- 运行本地开发：`pnpm dev`
- 验证结构：`pnpm validate:rule-structure`
- 评分规则库：`pnpm score:rules`
- 重新生成衍生产物：`pnpm generate:skills` 和 `pnpm generate:readme`

## 配合 MCP 使用

将支持 MCP 的 Agent 连接到 Front-End Checklist，用于前端代码审查、结构化规则查询、审计以及跨 React、Next.js、HTML、CSS、JavaScript、无障碍性、性能、SEO、安全、图片、隐私、国际化（i18n）和测试的修复工作流。

> [!TIP]
> 最佳首次使用方式：将支持 MCP 的 Agent 指向一个真实的组件、页面或公开 URL，并明确要求它优先使用 Front-End Checklist MCP 获取置信度最高的前端审查结果。部分客户端会延迟发现已安装的 MCP 工具，因此在 Prompt 中显式命名 server 会有帮助。
...
- 首次审查粘贴的 HTML、CSS、JavaScript、React 或 Next.js 代码时，优先使用 `review_code`
- 在给出前端无障碍性、性能、SEO、安全或图片优化建议前，先使用 `search_rules`
- 需要启动检查、无障碍性、SEO、安全和性能审计时，使用 `get_workflow` 或 `get_checklist_rules`
- 针对公开 `https://` 页面进行审计时使用 `audit_url`
...
当你希望在支持该功能的工具中复用审计工作流或获取特定规则的集中指导时，安装 Front-End Checklist skills。
...
- 全局审计入口点：[`skills/frontend-checklist-global/SKILL.md`](skills/frontend-checklist-global/SKILL.md)
- 聚焦规则 skill 示例：[`skills/https/SKILL.md`](skills/https/SKILL.md)

使用场景：

- 针对完整的 Front-End Checklist 规则库运行广泛的前端审计
- 使用 `https` 等聚焦型 skill 对单一关注点（如安全）进行审查
- 使用特定规则的 skills 了解某条规则的重要性及修复方法
