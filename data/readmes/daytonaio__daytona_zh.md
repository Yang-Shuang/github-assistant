...
Daytona 是一个用于运行 AI 生成代码和 Agent（智能体）工作流的、安全且弹性可扩展的基础设施运行时。我们的开源平台提供 [沙箱](https://www.daytona.io/docs/sandboxes/)，这些是完全隔离的、可组合的计算环境，拥有独立的内核、文件系统、网络栈以及分配的 vCPU、RAM 和磁盘空间。

沙箱是 Daytona 平台的核心组件，从代码到执行环境的启动时间不到 90 毫秒，支持运行 Python、TypeScript 和 JavaScript 等任何代码。基于 OCI/Docker 兼容性、大规模并行化和无限持久化能力构建，沙箱为 Agent（智能体）工作流提供了一致且可预测的运行环境。

Agent（智能体）和开发者通过 Daytona [SDKs](https://www.daytona.io/docs/#3-install-the-sdk)、[API](https://www.daytona.io/docs/tools/api/#daytona/) 和 [CLI](https://www.daytona.io/docs/tools/cli/) 以编程方式与沙箱交互。操作涵盖沙箱生命周期管理、文件系统操作、进程与代码执行，以及通过基础镜像、软件包和工具进行的运行时配置。我们的有状态环境 [快照](https://www.daytona.io/docs/snapshots/) 支持跨会话的持久化 Agent（智能体）操作，使 Daytona 成为 AI Agent（智能体）架构的理想基础。
