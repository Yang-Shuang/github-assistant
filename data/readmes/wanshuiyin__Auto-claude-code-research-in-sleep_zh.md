...
💡 *将 ARIS 作为基于技能的工作流使用于 [Claude Code](https://docs.anthropic.com/en/docs/claude-code) / [Codex CLI](skills/skills-codex/) / [Cursor](docs/CURSOR_ADAPTATION.md) / [Trae](docs/TRAE_ARIS_RUNBOOK_EN.md) / [Antigravity](docs/ANTIGRAVITY_ADAPTATION.md) / [GitHub Copilot CLI](docs/COPILOT_CLI_ADAPTATION.md) / [OpenClaw](docs/OPENCLAW_ADAPTATION.md)，或通过独立 CLI 获取完整体验——随你喜好！*

🌱 *ARIS 是一种方法论，而非平台。重要的是研究工作流——走到哪里带到哪里。*

🔥 *ARIS 原生适配任何 Agent 的 **ultracode-style deep mode** ——广度/火力覆盖层可适应运行时（Claude Code ultracode + Opus 4.8 上的工作流，Codex `spawn_agent`，或纯顺序执行），为三个角色提供支撑：**广度 · 跨模型评审 → 准确性 · 研究维基 → 记忆**。* *无论循环如何驱动，它都向相同的跨模型陪审团和研究维基报告——**它可以驱动，但无权裁决**。*

🤖 **AI agents：**请阅读 [`AGENT_GUIDE.md`](AGENT_GUIDE.md) ——该文件专为 LLM 消费设计，而非人类浏览。
