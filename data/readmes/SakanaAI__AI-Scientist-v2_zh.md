...
完全自主的科学研究系统正变得越来越强大，人工智能在改变科学发现方式方面发挥着关键作用。我们很高兴推出 The AI Scientist-v2，这是一个通用的端到端智能体（agentic）系统，它已生成首篇完全由 AI 撰写并通过同行评审的研讨会论文。

该系统自主生成假设、运行实验、分析数据并撰写科学手稿。与[其前身（AI Scientist-v1）](https://github.com/SakanaAI/AI-Scientist)不同，The AI Scientist-v2 摒弃了对人工编写模板的依赖，在机器学习（ML）领域具有通用性，并在实验管理器智能体（agent）的指导下采用渐进式智能体树搜索。

> **注意：**
> The AI Scientist-v2 并不一定能比 v1 生成更好的论文，尤其是在已有高质量起始模板的情况下。v1 遵循明确定义的模板，成功率较高；而 v2 采取更广泛、更具探索性的方法，成功率相对较低。当任务目标明确且基础扎实时，v1 效果最佳；而 v2 专为开放式科学探索设计。

> **警告！**
> 该代码库将执行由大语言模型（LLM）编写的代码。这种自主性伴随着各种风险和挑战，包括可能使用危险软件包、不受控制的网页访问以及意外生成进程的可能性。请确保在受控的沙盒环境（例如 Docker 容器）中运行。使用时请自行斟酌并承担相应风险。
