...
**Windows-MCP** 是一个轻量级、开源的项目，旨在实现 AI Agent（智能体）与 Windows 操作系统之间的无缝集成。作为 MCP 服务器，它填补了 LLM 与 Windows 操作系统之间的桥梁，使 Agent 能够执行**文件导航、应用控制、UI 交互、QA 测试**等任务。

mcp-name: io.github.CursorTouch/Windows-MCP

## Updates
- Windows-MCP 在 [Claude Desktop 扩展目录](https://claude.ai/directory) 的用户数已突破 `2M+`。 
- 试用基于 Windows-MCP 构建的 Agent **[🪟Windows-Use](https://pypi.org/project/windows-use/)**。
- Windows-MCP 现已发布至 [PyPI](https://pypi.org/project/windows-mcp/)（支持 `uvx windows-mcp`）
- Windows-MCP 已收录于 [MCP Registry](https://github.com/modelcontextprotocol/registry)
...
- **Seamless Windows Integration**  
  原生交互 Windows UI 元素、打开应用、控制窗口、模拟用户输入等。

- **Use Any LLM (Vision Optional)**
   与传统自动化工具不同，Windows-MCP 不依赖任何传统的计算机视觉技术或特定微调模型；它兼容所有 LLM，降低了复杂度和配置时间。

- **Rich Toolset for UI Automation**  
  包含基础键盘、鼠标操作以及捕获窗口/UI 状态的工具。

- **Lightweight & Open-Source**  
  依赖极少，易于部署，完整源代码遵循 MIT 许可证公开。

- **Customizable & Extendable**  
  轻松适配或扩展工具，以满足你独特的自动化或 AI 集成需求。

- **Real-Time Interaction**  
  典型操作延迟（例如从一次鼠标点击到下一次）通常在 **0.2 至 0.5 秒**之间，具体可能因活跃应用数量、系统负载以及 LLM 的推理速度而略有波动。

- **DOM Mode for Browser Automation**  
  State-Tool 提供专用的 `use_dom=True` 模式，专注于网页内容本身，过滤掉浏览器 UI 元素以实现更简洁高效的 Web 自动化。支持 Chrome、Edge 和 Firefox（Firefox 使用 IAccessible2 回退机制，因其未通过 UIA 暴露 `RootWebArea`）。
