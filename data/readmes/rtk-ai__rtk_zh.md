...
<p align="center">
  <strong>高性能 CLI 代理，可减少 60%-90% 的 LLM Token 消耗</strong>
</p>
...
rtk 会在命令输出进入你的 LLM 上下文之前对其进行过滤和压缩。单个 Rust 二进制文件，支持 100+ 命令，开销低于 10ms。

## Token 节省量（30分钟 Claude Code 会话）
...
> 估算基于中等规模的 TypeScript/Rust 项目。实际节省比例因项目而异。
...
> **Windows 用户注意**：解压 zip 文件并将 `rtk.exe` 放置于 PATH 中的某个位置（例如 `C:\Users\<你>\.local\bin`）。请通过**命令提示符**、**PowerShell** 或 **Windows Terminal** 运行 RTK——切勿双击 `.exe` 文件（它会闪退并关闭）。为了获得最佳体验，建议使用 [WSL](https://learn.microsoft.com/en-us/windows/wsl/install)，其中完整的钩子系统可原生工作。详见下方的 [Windows 设置](#windows)。
