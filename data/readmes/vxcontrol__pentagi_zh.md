...
> **加入社区！** 与安全研究人员、AI 爱好者以及同行伦理黑客建立联系。获取支持，分享见解，并随时了解 PentAGI 的最新动态。
...
- [功能特性](#features)
- [架构设计](#architecture)
  - [高级 Agent 监管](#advanced-agent-supervision)
- [快速开始](#quick-start)
...
PentAGI 是一款创新的自动化安全测试工具，利用尖端的人工智能技术。该项目专为信息安全专业人员、研究人员和爱好者设计，旨在提供强大且灵活的渗透测试解决方案。
...
- **安全且隔离**：所有操作均在完全隔离的沙盒化 Docker 环境中执行。
- **完全自主运行**：AI 驱动的 Agent 自动确定并执行渗透测试步骤，可选执行监控和智能任务规划以提高可靠性。
- **专业渗透工具**：内置包含 nmap、metasploit、sqlmap 等在内的 20+ 个专业安全工具套件。
- **智能记忆系统**：长期存储研究成果和成功的方法以供将来使用。
- **知识图谱集成**：基于 Graphiti 的知识图谱，使用 Neo4j 进行语义关系跟踪和高级上下文理解。
- **Web 情报**：内置通过 [scraper](https://hub.docker.com/r/vxcontrol/scraper) 提供的浏览器功能，用于从网络来源收集最新信息。
- **外部搜索系统**：集成先进的搜索 API，包括 [Tavily](https://tavily.com)、[Traversaal](https://traversaal.ai)、[Perplexity](https://www.perplexity.ai)、[DuckDuckGo](https://duckduckgo.com/)、[Google Custom Search](https://programmablesearchengine.google.com/)、[Sploitus Search](https://sploitus.com) 和 [Searxng](https://searxng.org)，以进行全面的信息收集。
- **专家团队**：具有专门用于研究、开发和基础设施任务的 AI Agent 委托系统，增强了可选的执行监控和智能任务规划功能，以便在较小模型下实现最佳性能。
- **全面监控**：详细的日志记录以及与 Grafana/Prometheus 的集成，用于实时系统观察。
...
- **自托管解决方案**：完全控制你的部署和数据。
- **灵活的认证**：支持 10+ LLM 提供商（[OpenAI](https://platform.openai.com/)、[Anthropic](https://www.anthropic.com/)、[Google AI/Gemini](https://ai.google.dev/)、[AWS Bedrock](https://aws.amazon.com/bedrock/)、[Ollama](https://ollama.com/)、[DeepSeek](https://www.deepseek.com/en/)、[GLM](https://z.ai/)、[Kimi](https://platform.moonshot.ai/)、[Qwen](https://www.alibabacloud.com/en/)、自定义）以及聚合器（[OpenRouter](https://openrouter.ai/)、[DeepInfra](https://deepinfra.com/)）。对于生产环境本地部署，请参阅我们的 [vLLM + Qwen3.5-27B-FP8 指南](examples/guides/vllm-qwen35-27b-fp8.md)。
- **API Token 认证**：用于程序化访问 REST 和 GraphQL API 的安全 Bearer token 系统。
- **快速部署**：通过 [Docker Compose](https://docs.docker.com/compose/) 轻松设置，包含全面的环境配置。

### 当前能力边界 (Current Capability Boundaries)

- PentAGI 目前是一个自主且辅助引导的渗透测试平台，而不是像 CALDERA 那样的预定义活动或攻击计划的漏洞模拟（BAS）或对手仿真产品。
- BAS 风格的 Agent 编写攻击脚本应被视为概念性或未来工作，而非当前已实现的功能。
- 当前的流程图 UI 支持 Web 视图、复制到剪贴板、Markdown 下载和 PDF 下载。JSON flow-report 导出目前不作为支持的输出格式记录。
- Provider 的灵活性通过内置提供商和自定义/OpenAI 兼容端点提供。请参阅 [自定义 LLM 提供商配置](#custom-llm-provider-configuration) 和 [vLLM + Qwen3.5-27B-FP8 指南](examples/guides/vllm-qwen35-27b-fp8.md)。
...
### 使用安装程序 (Using Installer - Recommended)

PentAGI 提供了一个带有基于终端 UI 的交互式安装程序，以简化配置和部署。安装程序将引导你完成系统检查、LLM 提供商设置、搜索引擎配置和安全加固。
...
- **macOS**: amd64 (Intel) [下载](https://pentagi.com/downloads/darwin/amd64/installer-latest.zip) | arm64 (M-series) [下载](https://pentagi.com/downloads/darwin/arm64/installer-latest.zip)
...
安装程序需要适当的权限才能正确操作并与 Docker API 交互。默认情况下，它使用 Docker socket (`/var/run/docker.sock`)，这需要以下任一方式：

- **选项 1（生产环境推荐）：以 root 用户运行安装程序：**
  ```bash
  sudo ./installer
  ```

- **选项 2（开发环境）：通过将你的用户添加到 `docker` 组来授予对 Docker socket 的访问权限：**
  ```bash
  # 将你的用户添加到 docker 组
  ...
  # 验证 Docker 访问（应无需 sudo）
  docker ps
  ```

  ⚠️ **安全说明：** 将用户添加到 `docker` 组会授予等同于 root 的权限。仅在对受控环境中的可信用户执行此操作。对于生产部署，建议使用无根 Docker 模式或以 sudo 运行安装程序。
...
### 当前 Web 设置覆盖范围 (Current Web Settings Coverage)

PentAGI Web 控制台在服务器运行后已经管理几个设置区域：

- **Settings -> Providers（设置 -> 提供商）**：创建、编辑、删除和测试用户定义的受支持提供商类型的配置文件。这些配置文件控制每个 Agent 的模型选择、运行时参数、推理选项和定价元数据。
- **Settings -> Prompts（设置 -> 提示词）**：管理系统、人类和工具提示模板。
- **Settings -> PentAGI API（设置 -> PentAGI API）**：创建和管理用于 REST 和 GraphQL 访问的 PentAGI Bearer token。
- **其他 UI 管理的首选项**：收藏的流程作为用户首选项存储，主题选择由主侧边栏/配置文件控件处理，而不是通过“设置”页面。

### 仍需在服务器上管理的配置 (Still Server-Managed)

以下配置区域仍需通过在服务器上设置环境变量、compose 文件或挂载的配置文件来进行设置：

- **LLM 凭证和连接详细信息**：API 密钥、端点、认证模式以及 OpenAI、Anthropic、Bedrock、Ollama、自定义提供商等后端特定的连接设置；config-path 设置仅适用于支持的地方，例如 `OLLAMA_SERVER_CONFIG_PATH` 和 `LLM_SERVER_CONFIG_PATH`。
- **搜索提供商凭证和选项**：诸如 `DUCKDUCKGO_*`、`GOOGLE_*`、`TAVILY_API_KEY`、`TRAVERSAAL_API_KEY`、`PERPLEXITY_*`、`SEARXNG_*` 和 `SPLOITUS_ENABLED` 等设置。
- **第三方集成**：Langfuse、Graphiti 和类似的外部服务仍然是服务器端配置。
- **MCP 服务器管理**：MCP 设置页面目前未作为实时 Web 控制台功能暴露。

**对于生产环境和高安全性：**

对于生产部署或安全敏感的环境，我们**强烈建议**使用分布式双节点架构，其中工作器操作在单独的服务器上隔离运行。这可以防止在你的主系统上执行不受信任的代码和访问网络问题。

**查看详细指南**：[Worker Node Setup](examples/guides/worker_node.md)

双节点设置提供：
- **隔离执行**：工作器容器在专用硬件上运行
- **网络隔离**：渗透测试的独立网络边界
- **安全边界**：带有 TLS 认证的 Docker-in-Docker
- **OOB 攻击支持**：带外技术的专用端口范围
